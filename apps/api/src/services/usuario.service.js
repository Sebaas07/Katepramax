...
const usuarioService = (app) => {
  ...
  return {
    ...
    create: async (data, creadoPorId) => {
      const { nombreCompleto, usuario, contrasena, rol, telefono, sedeId, sedesIds, activo } = data;

      const existeUsuario = await repo.findByUsuario(usuario);
    ...
    update: async (id, data, actualizadoPorId) => {
      ...
      const campos = {};
      const permitidos = ["nombreCompleto", "usuario", "telefono", "rol", "sedeId", "activo"];
