...
describe("egresoService.registrar", () => {
  const body = { fecha: "2026-05-05", semana: 18, sedeId: 1, concepto: "Transporte", total: 80000 };

  it("debería lanzar AppError 404 si la sede no existe", async () => {
    prisma.sede.findUnique.mockResolvedValue(null);

    await expect(egresoSvc.registrar(appMock, body)).rejects.toMatchObject({
  ...
  it("debería pasar fecha como Date y total como número a repo.crear", async () => {
    ...
    await egresoSvc.registrar(appMock, body);
...
describe("egresoService.obtenerPorId", () => {
  it("debería retornar el egreso si existe", async () => {
    ...
    const result = await egresoSvc.obtenerPorId(appMock, 1);
  ...
  it("debería lanzar AppError 404 si no existe", async () => {
    ...
    await expect(egresoSvc.obtenerPorId(appMock, 999)).rejects.toMatchObject({ statusCode: 404 });
  });
});
...
describe("egresoService.editar", () => {
  it("debería lanzar AppError 404 si no existe", async () => {
    ...
    await expect(egresoSvc.editar(appMock, 999, { total: 90000 })).rejects.toMatchObject({
      statusCode: 404,
    });
  });
  ...
  it("debería convertir total a número", async () => {
    ...
    await egresoSvc.editar(appMock, 1, { total: "90000" });
  ...
  it("no debería incluir campos no enviados en el update", async () => {
    ...
    await egresoSvc.editar(appMock, 1, { concepto: "Arriendo" });
...
describe("egresoService.borrar", () => {
  it("debería lanzar AppError 404 si no existe", async () => {
    ...
    await expect(egresoSvc.borrar(appMock, 999)).rejects.toMatchObject({ statusCode: 404 });
  });
  ...
  it("debería llamar repo.eliminar si existe", async () => {
    ...
    await egresoSvc.borrar(appMock, 1);
...
describe("egresoService.resumenPorSede", () => {
  it("debería calcular totalGeneral sumando todas las sedes", async () => {
    ...
    const result = await egresoSvc.resumenPorSede(appMock, 18);
...
describe("egresoService.resumenPorConcepto", () => {
  it("debería mapear concepto, registros y total", async () => {
    ...
    const result = await egresoSvc.resumenPorConcepto(appMock, 18);
...
describe("ingresoService.registrar", () => {
  const body = { fecha: "2026-05-05", semana: 18, sedeId: 1, efectivo: 300000, cuentas: 150000 };

  it("debería lanzar AppError 404 si la sede no existe", async () => {
    prisma.sede.findUnique.mockResolvedValue(null);

    await expect(ingresoSvc.registrar(appMock, body)).rejects.toMatchObject({
      statusCode: 404, message: expect.stringMatching(/sede/i),
    });
  });
  ...
  it("debería calcular total = efectivo + cuentas en el servidor", async () => {
    ...
    await ingresoSvc.registrar(appMock, body);
  ...
  it("debería usar cuentas=0 si no se pasa", async () => {
    prisma.sede.findUnique.mockResolvedValue(sedeMock);
    prisma.ingreso.create.mockResolvedValue({ ...ingresoMock, cuentas: 0, total: 300000 });

    await ingresoSvc.registrar(appMock, { ...body, cuentas: undefined });
...
describe("ingresoService.obtenerPorId", () => {
  it("debería retornar el ingreso si existe", async () => {
    ...
    const result = await ingresoSvc.obtenerPorId(appMock, 1);
  ...
  it("debería lanzar AppError 404 si no existe", async () => {
    ...
    await expect(ingresoSvc.obtenerPorId(appMock, 999)).rejects.toMatchObject({ statusCode: 404 });
  });
});
...
describe("ingresoService.editar", () => {
  it("debería lanzar AppError 404 si no existe", async () => {
    ...
    await expect(ingresoSvc.editar(appMock, 999, { efectivo: 400000 })).rejects.toMatchObject({
      statusCode: 404,
    });
  });
  ...
  it("debería recalcular total usando cuentas actuales si solo cambia efectivo", async () => {
    ...
    await ingresoSvc.editar(appMock, 1, { efectivo: 400000 });
  ...
  it("debería recalcular total usando efectivo actual si solo cambia cuentas", async () => {
    prisma.ingreso.findUnique.mockResolvedValue(ingresoMock);
    prisma.ingreso.update.mockResolvedValue({ ...ingresoMock, cuentas: 200000, total: 500000 });

    await ingresoSvc.editar(appMock, 1, { cuentas: 200000 });
  ...
  it("no debería actualizar total si solo cambia observacion", async () => {
    ...
    await ingresoSvc.editar(appMock, 1, { observacion: "Nota" });
...
describe("ingresoService.borrar", () => {
  it("debería lanzar AppError 404 si no existe", async () => {
    ...
    await expect(ingresoSvc.borrar(appMock, 999)).rejects.toMatchObject({ statusCode: 404 });
  });
  ...
  it("debería llamar repo.eliminar si existe", async () => {
    ...
    await ingresoSvc.borrar(appMock, 1);
...
describe("ingresoService.resumenPorSede", () => {
  it("debería calcular totalGeneral sumando todas las sedes", async () => {
    prisma.ingreso.groupBy.mockResolvedValue([
      ...
      { sedeId: 2, _sum: { efectivo: 200000, cuentas: 50000, total: 250000 },  _count: { id: 1 } },
    ]);
    prisma.sede.findMany.mockResolvedValue([
      { id: 1, nombre: "Bogotá" },
      { id: 2, nombre: "Villavicencio" },
    ...
    const result = await ingresoSvc.resumenPorSede(appMock, 18);
